...
const { applyToTextNodes, appendMeta, appendLink } = require('./domHelpers');
...
const buildChapter = templates => files => files.map(file => ({
  ...
  content: templates(
    { 
      content: renderContent(file.content), 
      meta: Object.assign({}, file.meta, { 
        section: file.name, sectionNumber: parseInt(file.name) 
      })
    },
...
const domEdits = (metadata, bibItems, dictionaryItems) => files => files.map((file) => {
  ...
  mapper.build(dom);
  typo.domAddNbsp(dom);
  dictionary.domAddLinks(dictionaryItems)(dom);
...
const build = (content, metadata, templates, bibItems, dictionaryItems) => pipe(
  extractFrontMatter,
  addNavigation,
  ...
  mapper.addCounts,
  dictionary.mapItems)([content.license, content.index, ...content.chapters]);

...
const renderLocations = (map) => {
  ...
  return map.reduce((links, loc) => {
    const chapter = Number.isNaN(parseInt(loc.chapter, 10))
      ? loc.chapter
      : parseInt(loc.chapter, 10);
    links.push(`<a href="${loc.chapter}.html#idea${loc.idea}">${chapter}:${loc.idea}</a>`);
    return links;
  }, []).join(', ');
};
...
const build = (terms, meta, files, templates) => {
  ...
  persistence.saveDictionary(templates(file, 'default'));
};

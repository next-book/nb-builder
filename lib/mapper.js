...
const splitIdeas = (/* config */) => text => text.split('\n');

const parseIdeas = (config, dom, childNodes) => childNodes.reduce((ideas, node) => {
  const splitIdeasFn = splitIdeas(config);
  ...
  if (node.nodeType === 3) {
    splitIdeasFn(node.textContent).map((part, index) => {
...
const mapIdeas = (config, dom) => {
  ...
  Array.prototype.map.call(els, (el) => {
    ...
    const ideas = parseIdeas(config, dom, childNodes);
    el.innerHTML = '';
    ideas.map((r) => {
